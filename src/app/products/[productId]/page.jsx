            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black">
                ...
                <div className="flex items-center justify-center h-[50vh]">
                    <div className="text-center">
                        ...
                        <p className="text-gray-400 mt-4 text-lg">Loading product details...</p>
                    </div>
                </div>
            </div>
        );
        ...
        if (loadingRecommendations) {
            return (
                <div className="mt-10 px-6">
                    <h2 className="text-2xl font-semibold text-white mb-4">Recommended Products</h2>
                                            ...
                                            onClick={() => {
                                                dispatch(
                                                    isFavorite(productData.id)
                                                        ? removeFavorite({ userId, productId: productData._id })
                                                        : addFavorite({ userId, product: productData })
                                                );
                                                notify(
                                                    productData.title,
                                                    isFavorite(productData.id) ? "removed from" : "added to"
                                                );
                                            }}
                                            className="absolute top-3 right-3 p-2 rounded-full bg-white/10 hover:bg-white/20"
                                        >
                                            <Heart
                                                className={`w-5 h-5 ${isFavorite(productData.id)
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex items-center gap-1">
                                        {[...Array(5)].map((_, i) => (
                                            <Star
                                                key={i}
                                                className={`w-5 h-5 ${i < Math.floor(productData.rating?.rate || 0)
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        >
                                            <ShoppingCart className={`w-5 h-5 ${isCart(productData.id)
                                                    ? "fill-white text-white"
                                                    : "text-white"
                                                }`} />
                                            {isCart(productData.id) ? "Remove from Cart" : "Add to Cart"}
                                        </button>
                                    </div>
                                </div>
                            </div>
